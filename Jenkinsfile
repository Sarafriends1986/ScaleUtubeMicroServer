   ...
   stage('Docker Build') {
		...
		try {
		  if (isUnix()) {
			 ...
			 sh 'pwd'
			 sh 'ls -ltr'
			 sh 'docker build -t sarafriends1986/scale-utube-micro-node:v_00.00.002 .'
   ...
   stage('Docker Image Upload') {
		...
		try {
		  if (isUnix()) {
			 ...
			 sh 'docker push sarafriends1986/scale-utube-micro-node:v_00.00.002'
   ...
   stage('Kubernetes Deployment') {
		...
		try {
		  if (isUnix()) {
			 ...
			 sh 'kubectl set image deployment/utube-micro-deployment utube-micro=sarafriends1986/scale-utube-micro-node:v_00.00.002'
